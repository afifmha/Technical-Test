...
from app.routers.api import router as ApiRouter
from app.routers.dependencies import getVectorStore, getRagServices
from app.core.interfaces import VectorStoreInterface
from app.services.vectoreStore import QdrantVectorStore
from app.services.ragServices import RAGService

app = FastAPI(title="Refactored RAG Demo")

app.include_router(ApiRouter, tags=["RAG"])

@app.get("/status")
def root(
    store: VectorStoreInterface = Depends(getVectorStore),
    rag_service: RAGService = Depends(getRagServices)
    ):
    
    using_qdrant = isinstance(store, QdrantVectorStore)
    
    return {
        "message": "RAG Service is running correctly",
        "system_info": {
            "database_type": "Qdrant" if using_qdrant else "In-Memory List",
            "document_count": store.document_count(),
            "workflow_ready": rag_service is not None
        }
    }
